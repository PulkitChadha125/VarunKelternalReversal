...
def write_to_signal_csv(action, option_price, option_contract, future_contract, future_price, lotsize):
    """
    Write trading signal to signal.csv file.
    
    Args:
        action: 'buy', 'sell', 'pyramiding trade buy', 'pyramiding trade sell', 'buyexit', 'sellexit'
        option_price: Price of the option (LTP or order price)
        option_contract: Option symbol (e.g., 'CRUDEOIL25NOV5300CE')
    ...
    try:
        ...
        with open(csv_file, 'a', newline='', encoding='utf-8') as file:
            ...
            if not file_exists:
                writer.writerow(['timestamp', 'Action', 'OptionPrice', 'OptionContract', 'FutureContract', 'Future price', 'Lotsize'])
            
            # Write data row
            writer.writerow([
                timestamp,
                action,
                f"{option_price:.2f}" if option_price is not None else "N/A",
                option_contract if option_contract else "N/A",
                future_contract if future_contract else "N/A",
                f"{future_price:.2f}" if future_price is not None else "N/A",
                lotsize
            ])
        
        print(f"[Signal CSV] Logged: {action} | Option: {option_contract} | Future: {future_contract} | OptionPrice: {option_price:.2f if option_price else 'N/A'} | FuturePrice: {future_price:.2f if future_price else 'N/A'} | Lotsize: {lotsize}")
    except Exception as e:
    ...
    try:
        ...
        if current_position == 'BUY':
            ...
            if prev_supertrend_trend is not None and prev_supertrend_trend == 1 and supertrend_trend == -1:
                ...
                if option_symbol and initial_exit_price is not None:
                    initial_entry = first_entry_price if first_entry_price else 0
                    initial_pnl = initial_exit_price - initial_entry if initial_entry > 0 else 0
                    write_to_order_logs(f"Position #1 (Initial):")
                    write_to_order_logs(f"  Option: {option_symbol}")
                    write_to_order_logs(f"  Entry Price: {initial_entry:.2f}")
                    write_to_order_logs(f"  Exit Price: {initial_exit_price:.2f}")
                ...
                # Log all pyramiding positions
                for pos_info in exited_positions:
                    write_to_order_logs(f"Position #{pos_info['position_num']} (Pyramiding):")
                    write_to_order_logs(f"  Option: {pos_info['option_symbol']}")
                    write_to_order_logs(f"  Entry Price: {pos_info['entry_price']:.2f}")
                    write_to_order_logs(f"  Exit Price: {pos_info['exit_price']:.2f}")
                    write_to_order_logs(f"  Quantity: {pos_info['quantity']}")
                    write_to_order_logs(f"  P&L per unit: {pos_info['pnl']:+.2f}")
                    write_to_order_logs(f"  Order ID: {pos_info['order_id']}")
                
                # Write to CSV for BUY exit (one log for all positions) - DO THIS FIRST before any potential errors
                # Calculate total lotsize (initial + all pyramiding positions)
                total_lotsize = lotsize * pyramiding_count if pyramiding_count > 0 else lotsize
                
                # Get option price for CSV logging
                exit_option_price = 0
                exit_option_contract = "N/A"
                if option_symbol:
                    exit_option_contract = option_symbol
                    try:
                        quote = get_option_quote(kite_client, option_exchange, option_symbol)
                        exit_option_price = quote.get('last_price', None)
                        if exit_option_price is not None:
                            exit_option_price = float(exit_option_price)
                        else:
                            exit_option_price = initial_exit_price if initial_exit_price else 0
                    except:
                        exit_option_price = initial_exit_price if initial_exit_price else 0
                else:
                    # No option symbol (order was rejected previously), use initial_exit_price if available
                    exit_option_price = initial_exit_price if initial_exit_price else 0
                
                # Always log to CSV regardless of order success/failure
                write_to_signal_csv(
                    action='buyexit',
                    option_price=exit_option_price,
                    option_contract=exit_option_contract,
                    future_contract=future_symbol,
                    future_price=ha_close,
                    lotsize=total_lotsize
                )
        ...
        if current_position == 'SELL':
            ...
            if prev_supertrend_trend is not None and prev_supertrend_trend == -1 and supertrend_trend == 1:
                ...
                # Write to CSV for SELL exit (one log for all positions) - DO THIS FIRST before any potential errors
                # Calculate total lotsize (initial + all pyramiding positions)
                total_lotsize = lotsize * pyramiding_count if pyramiding_count > 0 else lotsize
            ...
            if pyramiding_distance > 0 and pyramiding_number > 0 and first_entry_price is not None:
                ...
                if pyramiding_count < max_positions:
                    ...
                    if should_add_pyramiding:
                        ...
                        action_name = 'pyramiding trade buy' if current_position == 'BUY' else 'pyramiding trade sell'
                        write_to_signal_csv(
                            action=action_name,
