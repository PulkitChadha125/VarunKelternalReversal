        ...
        if current_position == 'BUY':
            ...
            if prev_supertrend_trend is not None and prev_supertrend_trend == 1 and supertrend_trend == -1:
                ...
                if option_symbol and option_exchange and kite_client:
                    ...
                    # Log when exit is triggered but no option symbol (order was rejected previously)
                    if not option_symbol:
                        write_to_order_logs(f"EXIT TRIGGERED: BUY position but no option_symbol (order was likely rejected) | Symbol: {future_symbol} | Supertrend flipped from GREEN (1) to RED (-1)")
                
                # Log exit and update position
                option_display = option_symbol if option_symbol else "N/A (Order was rejected)"
                write_to_order_logs(f"ORDER EXITED: BUY position | Symbol: {future_symbol} | Option: {option_display} | Supertrend flipped from GREEN (1) to RED (-1) | Prev: {prev_supertrend_trend} -> Current: {supertrend_trend}")
        ...
        if current_position == 'SELL':
            ...
            if prev_supertrend_trend is not None and prev_supertrend_trend == -1 and supertrend_trend == 1:
                ...
                if option_symbol and option_exchange and kite_client:
                    ...
                    # Log when exit is triggered but no option symbol (order was rejected previously)
                    if not option_symbol:
                        write_to_order_logs(f"EXIT TRIGGERED: SELL position but no option_symbol (order was likely rejected) | Symbol: {future_symbol} | Supertrend flipped from RED (-1) to GREEN (1)")
                
                # Log exit and update position
                option_display = option_symbol if option_symbol else "N/A (Order was rejected)"
                write_to_order_logs(f"ORDER EXITED: SELL position | Symbol: {future_symbol} | Option: {option_display} | Supertrend flipped from RED (-1) to GREEN (1) | Prev: {prev_supertrend_trend} -> Current: {supertrend_trend}")
